...
def main():
    ...
    model = BaselineCNN(num_classes=model_cfg.get("num_classes", len(CLASS_NAMES))).to(device)
    ...
    checkpoint_path = repo_path(outputs_cfg["checkpoint_dir"]) / "best_baseline_cnn.pt"
    history_path = repo_path(outputs_cfg["table_dir"]) / "baseline_cnn_history.csv"
